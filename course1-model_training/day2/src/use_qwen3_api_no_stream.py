...
system_prompt = '''
你是一位专业的文本分析专家，擅长从给定的原始文本中挖掘出准确、简洁且信息丰富的问答对（QA pairs）。
你的任务是：

仔细阅读用户提供的原始文本；
从中提取 5 个具有代表性的问答对；
每个问答对应聚焦一个独立的事实或关键信息点，避免重复；
问题应自然、清晰、符合人类提问习惯；答案应准确、简洁，直接来自或可明确推断自原文；
严格以 JSONL 格式输出：每行一个 JSON 对象，包含两个字段 "question" 和 "answer"，不包含任何额外说明、标题或 Markdown；
禁止添加序号、注释、解释或换行符；只输出 5 行纯 JSONL。
你将始终以专业、客观、精准的态度完成任务。
'''

user_prompt = """
请基于以下文本，扮演一位专业文本分析专家，从中挖掘出 5 个高质量的问答对。
要求：

每个问答对必须基于文本内容，答案需忠实原文；
问题应自然、具体、有信息量；
要求最终格式为一个列表，列表每个元素是{{"question": "…", "answer": "…"}}，示例：
[
    {{"question": "…", "answer": "…"}},
    {{"question": "…", "answer": "…"}},
    ...
]
不要输出任何其他内容（如说明、序号、空行等）。
文本如下：{text}
""".format(text=text)
...
res_list = eval(result)

import json
with open("qa_paris.jsonl", "w") as f:
